...
class QLearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # Q(s,a) ← Q(s,a) + α(r + γ max Q(s',a') for a' in actions − Q(s,a))
        # Q(s,a) ← Q(s,a) + α(r + γ compute utility (s') − Q(s,a))
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        # Q(s,a) = sum (wia * xi)
        # where is the the weight of the feature xi in and x1,x2,...,xn are the features of the state s.
        return sum([self.weights[action][feature] * features[feature] for feature in features])
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            q_next = self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))
        ...
        for feature in features:
            # wia ← wia + α(r + γ compute utility (s') − Q(s,a))wi
            updated_weights[action][feature] = self.weights[action][feature] + self.learning_rate * (reward + self.discount_factor * q_next - q) * features[feature] 
